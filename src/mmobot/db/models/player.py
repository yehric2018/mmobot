...
from mmobot.constants import (
    FIGHTING_OFFENSE_RATIO,
    FIGHTING_DEFENSE_RATIO,
    EVASION_DEFENSE_RATIO,
    MOBILITY_DEFENSE_RATIO,
    WEAPON_OFFENSE_RATIO,
    WEAPON_DEFENSE_RATIO,
    WEAPONLESS_LETHALITY
)
...
class Player(Agent):
    ...
    def get_offense_score(self):
        ...
        fighting_skill = self.get_skill_level("fighting") * FIGHTING_OFFENSE_RATIO
        weapon_skill *= WEAPON_OFFENSE_RATIO
    ...
    def get_defense_score(self):
        ...
        if weapon_instance is not None:
            weapon_range = weapon_instance.item.range
            weapon_skill = self.get_weapon_skill(weapon_instance.item)
        fighting_skill = self.get_skill_level('fighting') * FIGHTING_DEFENSE_RATIO
        weapon_skill *= WEAPON_DEFENSE_RATIO
        evasion_skill = self.get_skill_level('evasion') * EVASION_DEFENSE_RATIO
        mobility = (self.mobility - 100) * MOBILITY_DEFENSE_RATIO
        raw_score = fighting_skill + weapon_skill + evasion_skill + weapon_range + mobility
        return raw_score * self.hp_endurance_ratio()
    ...
    def get_weapon_skill(self, weapon=None):
        if weapon is None and self.equipped_weapon is None:
            weapon_instance = self.get_equipped_weapon()
            if weapon_instance is None:
                return 0
            weapon = weapon_instance.item
